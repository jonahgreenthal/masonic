...
public abstract class Standalone {
	...
	public static void createDataSource() throws ClassNotFoundException, NamingException {
		...
		lclC.bind(
			"java:comp/env/MasonicDataSource",
			DirectConnectionPoolFactory.getInstance().create(
				"jdbc:postgresql://127.0.0.1:5432/masonic",
				MasonicConfiguration.getInstance().getString("DATABASE_USERNAME"),
				MasonicConfiguration.getInstance().getString("DATABASE_PASSWORD")
			)
		);
	}
	...
	public static void createConsoleLogger(final Priority argPriority) {
		...
		lclAppender.setName("MasonicAppender");
