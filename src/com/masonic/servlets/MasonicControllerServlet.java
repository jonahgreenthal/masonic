...
public abstract class MasonicControllerServlet extends ControllerServlet {
	...
	protected static final org.apache.log4j.Logger ourLogger = org.apache.log4j.Logger.getLogger(MasonicControllerServlet.class);
	 ...
	 * 
	 * To detect problems with this, whenever an MasonicControllerServlet (or its subclass) starts up, we check to see if the
	...
	/** In general, invoking an MasonicControllerServlet does all of the things a ControllerServlet does, then translates the
	 * username into an Masonic Account so it can pass that to processInternal (which should be overridden to do the actual
	 * work.
	 * 
	 ...
	 * 
	 * MasonicControllerServlets should not try to send HTML (or anything else) to the user (which is why the internal
